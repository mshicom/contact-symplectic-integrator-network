...
def plot_potential(savefig=False):
    ...
    q_cdl = torch.tensor(cdl_data[:, 0:1], dtype=torch.float32).to(device)
    ...
    if _train_vin:
        device_vin = next(vin_model.parameters()).device
        q_vin = torch.tensor(vin_data[:, 0:1], dtype=torch.float32).to(device_vin)
    ...
    contact_in = torch.tensor(np.concatenate([cdl_data[1:, 0:1], cdl_data[:-1, 1:2]], 1), dtype=torch.float32).to(device)
    plt.plot(t[1:], cdl_model.contact(contact_in).detach().cpu().numpy()[:, 0], label='CD-Lagrange', linewidth=4.0)
    plt.ylim((-0.1, 1))
    plt.xlabel('Time in s')
    plt.legend()
    if savefig:
